...
c    ================================================================================================
        ...
        DIMENSION N(500),NZ(500),R(500),ERR(500),KZ(200)
   68   OPEN(UNIT=18,FILE='2020_r0_EE.dat',STATUS='OLD')
        OPEN(UNIT=19,FILE='ELE.in',STATUS='OLD')
        NN=186
        DO 10 I=1,NN
        READ(18,23)NZ(I),N(I),R(I),ERR(I)
   23   FORMAT(2X,I3,1X,I3,1X,F8.6,1X,F8.6)  
   10   CONTINUE
...
c                       program simply list the even even even radii (as given in 2020_r0_EE.DAT)
c-----------------------------------------------------------------------------------------------------
 21     NZ1=NZ1+2
        N1=N1+2
          DO 41 I=1,NN
              ...
              IF(I.EQ.NN)THEN
	         WRITE(*,70)
   70            Format(//,' The radius parameter does not exist in 
     12020_r0_EE.DAT data file.',/)
                GOTO 83
                STOP
              ENDIF
   41     CONTINUE
...
c-----------------------------------------------------------------------------------------------------
   25   AN2=N1+3
        NZ1=NZ1+2
	      ...
	      IF(I.EQ.NN)THEN
	         WRITE(*,81)
   81            Format(//,' The radius parameter could not be deduced a   
     1s one of the even-even input radius  parameter does not exist in t
     1he 2020_r0_EE.DAT data file.',/)    
	         GOTO 83
	         STOP
	      ENDIF
   42     CONTINUE
